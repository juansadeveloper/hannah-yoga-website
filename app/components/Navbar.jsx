...
function Navbar() {
  ...
  return (
    <main className='bg-gradient-to-r from-[#ede3dd] to-[#ede3dd] w-full h-[80px] flex justify-center fixed top-0 z-20'>
        ...
        <div className='max-w-[1580px] w-full h-full flex justify-between items-center px-5'>
            ...
            <ul className=' px-5 py-3 rounded-md max-lg:hidden flex items-center gap-3 text-accent/90  text-base font-medium'>
        ...
        {nav ? <AiOutlineClose size={30} className='mlg:hidden cursor-pointer text-accent z-10' onClick={toggleNav}/> : <HiOutlineMenuAlt4  size={26} className='md:hidden cursor-pointer text-accent' onClick={toggleNav} />}
        {nav ? 
            <ul className='lg:hidden absolute top-0 left-0 flex flex-col w-full z-0 bg-gradient-to-b from-[#EADDD4] to-[#F1EDEA]' onClick={toggleNav}>
                <div className='justify-center items-center flex flex-col h-screen pb-20  text-accent text-xl px-5'>
                  ...
                  <Link href="/retreats"><li className='hover:text-white/85 transition duration-300 rounded px-4 py-2 cursor-pointer'>RETREATS</li></Link>
